-- Per-year team names.
--
-- A team is one identity across years (a family, an organisation) so its history
-- links up, but the name it plays under can change year to year — these are
-- playful names, and a group may enter as "The Woodruffs" one year and "Woodruff
-- Lineage" the next. Previously the only name lived on `teams.name`, so renaming
-- rewrote every past leaderboard.
--
-- `golf_teams.display_name` is that year's name. NULL means "no special name this
-- year", falling back to the identity's `teams.name` — so existing rows keep the
-- name they already show and nothing needs back-filling.
--
--   teams.name              → the identity: what the team is called in general,
--                             edited on Admin → Teams, used by any year without
--                             its own name.
--   golf_teams.display_name → what this particular year's entry is called, edited
--                             in Team Builder.

alter table public.golf_teams
  add column if not exists display_name text;

comment on column public.golf_teams.display_name is
  'Name this team played under for this tournament year. NULL falls back to teams.name.';

-- Both public-facing views resolve the name the same way. Existing columns keep
-- their names, types and order so `create or replace` preserves grants and RLS;
-- leaderboard_view gains team_identity_name at the end for admin screens that need
...
select
  ...
  gt.tournament_id,
  tm.id as teams_id,
  -- The name this team played under that year.
...
create or replace view public.tee_times_view with (security_invoker='on') as
select
...
group by
  ...
  tt.hole_number, tt.notes, tt.updated_at, gt.display_name, tm.name, gt.team_number,
